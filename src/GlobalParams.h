...
// AR means "Already Reserved"
#define RT_AR_SAME_OUTPORT_SAME_OUTVC        -1
#define RT_AR_SAME_OUTPORT_OTHER_OUTVC       -2
#define RT_AR_OTHER_OUTPORT_SAME_OUTVC       -3
#define RT_AR_OTHER_OUTPORT_OTHER_OUTVC      -4
#define RT_OUTVC_BUSY                        -5
...
#define TOPOLOGY_MULTI_MESH    "MULTI_MESH"
...
#define TRAFFIC_LOCAL_CHIPLET  "TRAFFIC_LOCAL_CHIPLET"
#define TRAFFIC_ULOCAL	       "TRAFFIC_ULOCAL"
#define TRAFFIC_SINGLE_PACKET  "TRAFFIC_SINGLE_PACKET"
#define TRAFFIC_DRAIN          "TRAFFIC_DRAIN"
...
typedef struct {
    ...
    unsigned mesh_id;
    map<string, vector<pair<int,int>>> upward_links_placement;
    map<string, vector<pair<int,int>>> upward_links_selection;
    map<string, vector<pair<int,int>>> downward_links_placement;
    map<string, vector<pair<int,int>>> downward_links_selection;
    // Intra-stack vertical links for a single mesh (local IDs).
    vector<int> intra_links_placement;
    // Mapping [src_router_local_id, selected_tsv_local_id].
...
struct GlobalParams {
    ...
    static int traffic_single_packet_src_id;
    static string traffic_single_packet_src_chip;
    static int traffic_single_packet_dst_id;
    static string traffic_single_packet_dst_chip;
    static string config_filename;
    static string power_config_filename;
    static int clock_period_ps;
    static int simulation_time;
    static int drain_time;
