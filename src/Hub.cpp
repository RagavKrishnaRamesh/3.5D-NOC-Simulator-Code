...
void Hub::rxPowerManager()
{
	...
	for (int port=0;port<num_ports;port++)
	{
		if (!buffer_to_tile[port][DEFAULT_VC].IsEmpty() ||
			antenna2tile_reservation_table.isNotReserved(port, DEFAULT_VC))
...
void Hub::txPowerManager()
{
	for (unsigned int i=0;i<txChannels.size();i++)
	{
		...
		if (!init[i]->buffer_tx.IsEmpty() ||
			tile2antenna_reservation_table.isNotReserved(i, DEFAULT_VC) )
		{
			power.leakageAntennaBuffer();
...
void Hub::antennaToTileProcess()
{
	...
	for (unsigned int i = 0; i < rxChannels.size(); i++)
	{
		...
		if (!(target[channel]->buffer_rx.IsEmpty()))
		{
			...
			if (received_flit.flit_type==FLIT_TYPE_HEAD)
			{
				...
				TReservation r;
				r.input = channel;
				r.vc_in  = received_flit.vc_id;
				r.vc_out = received_flit.vc_id;
				...
				int rt_status = antenna2tile_reservation_table.checkReservation(r,dst_port,dummy);
	...
	for (unsigned int i = 0; i < rxChannels.size(); i++)
	{
		...
		vector<ReservationTable::res_t> reservations =
                    antenna2tile_reservation_table.getReservations(channel);

		if (reservations.size()!=0)
		{
			int rnd_idx = rand()%reservations.size();

			int port   = get<RES_PORT_OUT>(reservations[rnd_idx]);
			...
			if (!(target[channel]->buffer_rx.IsEmpty()))
			{
				...
				if ( !buffer_to_tile[port][vc_in].IsFull() )
				{
					target[channel]->buffer_rx.Pop();
					power.antennaBufferPop();
					LOG << "*** [Ch" << channel << "] Moving flit  " << received_flit << " from buffer_rx to buffer_to_tile[" << port <<"][" << vc_in << "]" << endl;

					buffer_to_tile[port][vc_in].Push(received_flit);
					...
					LOG << "Full buffer_to_tile[" << port <<"][" << vc_in << "]" << ", cannot store " << received_flit << endl;
			}
			else
			{
				// should be ok
...
void Hub::tileToAntennaProcess()
{
			...
			if (!buffer_from_tile[i][vc].IsEmpty())
			{
				...
				if (flit.flit_type == FLIT_TYPE_HEAD)
				{
					...
					LOG << "Checking reservation availability of Channel " << channel << " by Hub port[" << i << "][" << vc << "] for flit " << flit << endl;
                                        ...
                                        ReservationTable::res_t dummy;
					int rt_status = tile2antenna_reservation_table.checkReservation(r,channel,dummy);
	...
	{
		vector<ReservationTable::res_t> reservations =
                    tile2antenna_reservation_table.getReservations(i);
		...
		if (reservations.size()!=0)
		{
			...
			if (!buffer_from_tile[i][vc_in].IsEmpty())
			{
				Flit flit = buffer_from_tile[i][vc_in].Front();
				// powerFront already accounted in 1st phase

				assert(r_from_tile[i][vc_in] == DIRECTION_WIRELESS);
				...
				if (channel != NOT_RESERVED)
				{
					if (!(init[channel]->buffer_tx.IsFull()) )
					{
						...
						LOG << "Flit " << flit << " moved from buffer_from_tile["<<i<<"]["<<vc_in<<"]  to buffer_tx["<<channel<<"] " << endl;
					}
					else
					{
				...
				{
					LOG << "Forwarding: No channel reserved for input port [" << i << "][" << vc_in << "], flit " << flit << endl;
				}
			}
