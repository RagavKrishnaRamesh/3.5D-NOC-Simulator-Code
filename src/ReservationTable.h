...
class ReservationTable {
  public:
    // RES_PORT_OUT, RES_VC_IN, RES_VC_OUT
    using res_t = tuple<int, int, int>;

    ReservationTable();

    inline string name() const {return "ReservationTable";};

    // Check no reservation has been done for the same input port / vc_in
    // requesting a new entry, and if not, check that the output port / vc_out
    // is available.
    // Returns a ReservationTable status value and if applicable, the
    // reservation already done for the same input port / vc_in, as an out
    // parameter.
    int checkReservation(const TReservation& r, const int port_out,
                         res_t& same_input_blocking_entry);
    ...
    vector<res_t> getReservations(const int port_in);

    // update the index of the reservation having highest priority in the current cycle
    void updateIndex();

    // check whether <port_out, vc_out> has no reservations
    bool isNotReserved(const int port_out, int vc_out);
