...
RouteEntry Selection_BUFFER_LEVEL::apply(Router * router,
                                         const vector<RouteEntry> & directions,
                                         const RouteData & route_data)
{
    vector<RouteEntry> best_dirs;
    int max_free_slots = 0;
    for (unsigned int i = 0; i < directions.size(); i++)
    {
	bool available = false;

        int dir = directions[i].first;
        int vc  = directions[i].second;
	int free_slots = router->free_slots_neighbor[dir].read().mask[vc];

	try {
	    available = router->reservation_table.isNotReserved(dir, vc);
	}
	catch (int error)
	{
