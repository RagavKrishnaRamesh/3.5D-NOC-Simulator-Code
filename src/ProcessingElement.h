...
SC_MODULE(ProcessingElement)
{
    ...
    sc_in <TFreeSlots> free_slots_neighbor;

    // Registers
    const int local_id;  // Unique Id for any topology except MULTI_MESH
    const int global_id; // Unique Id for any topology (including MULTI_MESH)
    ...
    Packet trafficSinglePacket();	// used for testing traffic
    ...
    Packet trafficLocalChiplet();	// Random with per chiplet locality
    ...
    using SC_CURRENT_USER_MODULE = ProcessingElement;
    // Constructor
    ProcessingElement(const sc_module_name& name, Tile& tile);

    static void listAvailablePEs();
    const Tile & tile;
    static vector<int> multi_noc_pe_list;
    static map<unsigned, vector<int>> per_chiplet_pe;
    bool disable_pe;

    random_device rd;
    mt19937 gen;
    discrete_distribution<unsigned> d;
};
