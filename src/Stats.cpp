...
void Stats::configure(const NoC & noc, const int node_id,
                      const double _warm_up_time)
{
    ...
    flit_injected_inter[noc.name()].resize(MAX_VIRTUAL_CHANNELS, 0);
    flit_injected_intra[noc.name()].resize(MAX_VIRTUAL_CHANNELS, 0);

    flit_received_inter[noc.name()].resize(MAX_VIRTUAL_CHANNELS, 0);
    flit_received_intra[noc.name()].resize(MAX_VIRTUAL_CHANNELS, 0);

    cumulated_nb_flits_intra[noc.name()].resize(MAX_VIRTUAL_CHANNELS, 0);
    cumulated_nb_flits_inter[noc.name()].resize(MAX_VIRTUAL_CHANNELS, 0);
#endif // DEBUG_VC_UTILIZATION
}

void Stats::sentFlit(const double emission_time,
                     const Flit & flit)
{
    if (emission_time - GlobalParams::reset_time < warm_up_time)
	return;
...
unsigned int Stats::getReceivedFlits() const
{
    int n = 0;
...
unsigned int Stats::getSentFlits() const
{
    int n = 0;
...
void Stats::updateVCCumulatedNumberOfFlits(const Router & router)
{
    const string noc_name = router.getNoC().name();

    for (size_t i = 0; i < DIRECTIONS + 2; i++) {
        for (size_t vc = 0; vc < MAX_VIRTUAL_CHANNELS; vc++) {
            for (auto const & flit : router.buffer[i][vc]) {
                if (isIntraNoC(router, flit))
                    cumulated_nb_flits_intra[noc_name][vc]++;
                else
                    cumulated_nb_flits_inter[noc_name][vc]++;
            }
        }
    }
}

void Stats::updateVCInjectedFlits(const Router & router, const Flit & flit)
{
    const string noc_name = router.getNoC().name();

    if (isIntraNoC(router, flit)) {
        flit_injected_intra[noc_name][flit.vc_id]++;
        return;
    }
    flit_injected_inter[noc_name][flit.vc_id]++;
}

void Stats::updateVCReceivedFlits(const Router & router, const Flit & flit)
{
    const string noc_name = router.getNoC().name();

    if (isIntraNoC(router, flit)) {
        flit_received_intra[noc_name][flit.vc_id]++;
        return;
    }
    flit_received_inter[noc_name][flit.vc_id]++;
}

bool Stats::isIntraNoC(const Router & router, const Flit & flit) const {
    return router.getNoC().getGlobalIdRange().isIncluded(flit.dst_id) &&
           router.getNoC().getGlobalIdRange().isIncluded(flit.src_id);
}
