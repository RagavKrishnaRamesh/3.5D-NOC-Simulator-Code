...
void Buffer::UpdateFront(const Flit & flit)
{
  if (IsEmpty()) {
    Push(flit);
    return;
...
deque<Flit>::const_iterator Buffer::begin() const {
    return buffer.begin();
}

deque<Flit>::const_iterator Buffer::end() const {
    return buffer.end();
}

void Buffer::atomicFlowControlCheck() const
{
    for (size_t buffer_idx = 0; buffer_idx < buffer.size(); buffer_idx++)
        if (buffer[buffer_idx].flit_type == FLIT_TYPE_HEAD)
            sc_assert(buffer_idx == 0);
}
