...
inline ostream & operator <<(ostream & os, const TFreeSlots & fs)
{
    os << "[" ;
    for (int j = 0; j < GlobalParams::n_virtual_channels; j++)
	os << fs.mask[j] << " ";
...
inline void sc_trace(sc_trace_file * &tf, const TFreeSlots & fs, string & name)
{
    for (int j = 0; j < GlobalParams::n_virtual_channels; j++)
	sc_trace(tf, fs.mask[j], name + "VC "+to_string(j));
}
...
inline Coord id2Coord(int id)
{
    ...
    if (GlobalParams::topology == TOPOLOGY_MESH)
    {
        ...
        coord.y = (id - (coord.z * GlobalParams::mesh_dim_x * GlobalParams::mesh_dim_y)) / GlobalParams::mesh_dim_x;
        coord.x = (id - (coord.z * GlobalParams::mesh_dim_x * GlobalParams::mesh_dim_y)) % GlobalParams::mesh_dim_x;
...
inline int coord2Id(const Coord & coord)
{
    ...
    if (GlobalParams::topology == TOPOLOGY_MESH)
    {
        id = coord.z * (GlobalParams::mesh_dim_x * GlobalParams::mesh_dim_y) +
             coord.y *  GlobalParams::mesh_dim_x + coord.x;
...
inline int xgetId(int x, int y, int z) {
    assert(GlobalParams::topology == TOPOLOGY_MESH);
    return z * (GlobalParams::mesh_dim_y * GlobalParams::mesh_dim_x) +
           y * GlobalParams::mesh_dim_x + x;
}
