...
vector<RouteEntry> Routing_ODD_EVEN::route(Router * router,
                                           const RouteData & routeData)
{
    ...
    int e0, e1;
    int vc_default = routeData.vc_in;
    ...
    if (e0 == 0) {
        if (e1 > 0)
            directions.push_back({DIRECTION_NORTH, vc_default});
        else
            directions.push_back({DIRECTION_SOUTH, vc_default});
    } else {
        if (e0 > 0) {
            if (e1 == 0)
                directions.push_back({DIRECTION_EAST, vc_default});
            else {
                if ((c0 % 2 == 1) || (c0 == s0)) {
                    if (e1 > 0)
            ...
            directions.push_back({DIRECTION_WEST, vc_default});
            if (c0 % 2 == 0) {
                if (e1 > 0)
                    directions.push_back({DIRECTION_NORTH, vc_default});
                if (e1 < 0)
