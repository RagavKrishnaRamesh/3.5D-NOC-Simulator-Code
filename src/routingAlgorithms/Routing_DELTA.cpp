...
vector<RouteEntry> Routing_DELTA::route(Router * router,
                                        const RouteData & routeData)
{
   ...
   // int switch_offset = GlobalParams::n_delta_tiles;
	...
	directions.push_back({0, routeData.vc_in}); // for inputs cores
    else
    { // for switch bloc
	...
	directions.push_back({direction, routeData.vc_in});
    }
    return directions;
}
