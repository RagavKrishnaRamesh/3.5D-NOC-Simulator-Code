...
vector<RouteEntry> Routing_TABLE_BASED::route(Router * router,
                                              const RouteData & routeData)
{
    ...
    vector<int> admissibleOutputs = admissibleOutputsSet2Vector(ao);
    vector<RouteEntry> directions_with_vc;
    transform(admissibleOutputs.begin(), admissibleOutputs.end(),
              back_inserter(directions_with_vc),
              [&](const int dir) {
                  return RouteEntry{dir, routeData.vc_in};
              });
    return directions_with_vc;
}
