...
def _select_redelf_random_tsv_local_index(router_local_idx,
...
def _select_capped_redelf_tsv_local_index(router_local_idx,
                                          tsv_local_indices,
                                          assignment_count,
                                          max_assignments_per_tsv):
    ...
    b1_candidates = [
        tsv for tsv in candidates
        if _is_south_or_due_east(tsv, router_local_idx)
    ]
    ...
    # B3; this is normally a no-op for the shared-pillar topology, but is
    # retained to match the Tabu implementation exactly.
...
def _select_tsv_local_index(router_local_idx, candidate_local_indices):
    ...
    if TSV_ASSIGNMENT_MODE == TSV_ASSIGNMENT_ELEVATOR_FIRST:
        return _select_elevator_first_tsv_local_index(
            router_local_idx,
            candidate_local_indices,
        )
    if TSV_ASSIGNMENT_MODE == TSV_ASSIGNMENT_REDELF_RANDOM:
        return _select_redelf_random_tsv_local_index(
            router_local_idx,
            candidate_local_indices,
        )
...
def _validate_tsv_assignment_single_3d(tsv_placement, no_routers, no_levels,
    ...
    for layer in range(no_levels):
        ...
        if tsv_list:
            # Keep TSV use balanced within this layer.  In REDELF mode the
            # selection order is B1 -> cap preference -> B2 -> B3, matching
            # TABU_2L_RAN_ES_sym_asym.py.
