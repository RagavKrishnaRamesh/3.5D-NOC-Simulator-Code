		    ...
		    string& error_msg)
{
  ...
  for (int i=0; i<repetitions; i++)
  {
    ...
    fout << "  (";
    for (uint i=0; i<aggr_conf.size(); i++)
      fout << setw(MATRIX_COLUMN_WIDTH) << ExtractFirstField(aggr_conf[i].second)
           << ", ";
...
#ifdef CSV_BACKEND
    for (uint i=0; i<aggr_conf.size(); i++)
      fout << ExtractFirstField(aggr_conf[i].second) << ", ";
    ...
    fout << setw(MATRIX_COLUMN_WIDTH) << sres.avg_delay << ", "
         ...
         << setw(MATRIX_COLUMN_WIDTH) << sres.total_energy << ", "
         << setw(MATRIX_COLUMN_WIDTH) << sres.rpackets << ", "
         << setw(MATRIX_COLUMN_WIDTH) << sres.rflits << endl;
#endif
  }
...
bool PrintPythonHeader(const string& fname,
    const TExplorerParams& eparams,
    const string& def_cmd_line, const string& conf_cmd_line, 
    ofstream& fout, 
    string& error_msg)
...
bool PrintCSVHeader(const string& fname,
    const TExplorerParams& eparams,
    const string& def_cmd_line, const string& conf_cmd_line, 
    ofstream& fout, 
    string& error_msg)
...
bool PrintPythonFunction(const string& mfname,
    ofstream& fout,
    string& error_msg)
{
  return true;
...
bool PrintCSVFunction(const string& mfname,
    ofstream& fout,
    string& error_msg)
{
  return true;
...
bool PrintPythonVariableBegin(const TParametersSpace& aggragated_params_space, 
    ofstream& fout, string& error_msg)
{
  fout << MATLAB_VAR_NAME << " = (" << endl;
  fout << "# ";
...
bool PrintCSVVariableBegin(const TParametersSpace& aggragated_params_space, 
    ofstream& fout, string& error_msg)
{
  for (TParametersSpace::const_iterator i=aggragated_params_space.begin();
       i!=aggragated_params_space.end(); i++)
...
bool GenPythonCode(const string& var_name,
    const int fig_no,
    const int repetitions, const int column,
    ofstream& fout, string& error_msg,
    const string y_label = "",
    const string x_label = "",
    const string fig_label = "")
{
  fout << var_name << " = {'x':[], 'y':[]};" << endl
       << "for i in range(int(rows/" << repetitions << ")):" << endl
       << "   ifirst = i * " << repetitions << endl
       << "   ilast  = ifirst + " << repetitions << " - 1" << endl
       << "   tmp = df.iloc[ifirst:ilast, " << column << "]" << endl
       << "   avg = tmp.mean()" << endl
       << "   " << var_name << "['x'].append(df.iloc[ifirst,0])" << endl
       << "   " << var_name << "['y'].append(avg)" << endl
       << endl;

  fout << "ax = axs[" << fig_no / 2 << "][" << fig_no % 2 << "]" << endl
       << endl;

  fout << "ax.plot("
       << var_name << "['x'], " << var_name << "['y'], 'ro-'"
       << ")"
       << endl;
...
bool PrintPythonVariableEnd(const int repetitions,
    ofstream& fout, string& error_msg)
{
  fout << ")" << endl << endl;

  fout << "import pandas as pd" << endl
       << "import matplotlib.pyplot as plt" << endl
       << endl;

  fout << "df = pd.DataFrame(" << MATLAB_VAR_NAME << ", columns=["
       << "'pir',"
       << "'avg_delay',"
       << "'throughput',"
       << "'max_delay',"
  ...
  fout << "rows = len(df.index)" << endl
       << "cols = len(df.columns)" << endl
       << endl;

  fout << "plt.style.use('ggplot')" << endl
       << endl;

  fout << "fig, axs = plt.subplots(2, 2, layout='constrained')" << endl
       << endl;

  if (!GenPythonCode(string(MATLAB_VAR_NAME) + "_delay", 0,
        repetitions, 1, fout, error_msg,
        "Cycles",
        "Packet Injection Rate (PIR)",
        "Average Delay versus PIR"))
    return false;

  if (!GenPythonCode(string(MATLAB_VAR_NAME) + "_throughput", 1,
        repetitions, 2, fout, error_msg,
        "Flits/Cycle",
        "Packet Injection Rate (PIR)",
        "Throughput versus PIR"))
    return false;

  if (!GenPythonCode(string(MATLAB_VAR_NAME) + "_maxdelay", 2,
        repetitions, 3, fout, error_msg,
        "Cycles",
        "Packet Injection Rate (PIR)",
        "Maximum delay versus PIR"))
    return false;

  if (!GenPythonCode(string(MATLAB_VAR_NAME) + "_totalenergy", 3,
        repetitions, 4, fout, error_msg,
        "Energy",
        "Packet Injection Rate (PIR)",
        "Energy versus PIR"))
...
//  if (!GenPythonCodeSaturationAnalysis(string(MATLAB_VAR_NAME), fout, error_msg))
//    return false;
...
#endif
#ifdef PYTHON_BACKEND
    string   fname  = mfname + ".py";
    if (!PrintPythonHeader(fname, eparams, 
          def_cmd_line, conf_cmd_line, fout, error_msg))
      return false;

    if (!PrintPythonFunction(mfname, fout, error_msg))
      return false;

    if (!PrintPythonVariableBegin(aggragated_params_space, fout, error_msg))
      return false;
#endif
#ifdef CSV_BACKEND
    string fname = mfname + ".csv";
    if (!PrintCSVHeader(fname, eparams, 
          def_cmd_line, conf_cmd_line, fout, error_msg))
      return false;

    if (!PrintCSVFunction(mfname, fout, error_msg))
      return false;

    if (!PrintCSVVariableBegin(aggragated_params_space, fout, error_msg))
      return false;
#endif
...
#endif
#ifdef PYTHON_BACKEND
    if (!PrintPythonVariableEnd(eparams.repetitions, fout, error_msg))
      return false;
#endif
  }
